...
Por outro lado, os coeficientes de correlação de Pearson entre as variáveis de esforço e a nota média foram marginalmente positivos e irrelevantes, indicando a ausência de uma relação linear significativa:

| Banco de Dados | rI^,R (Ingredientes vs. Nota) | rT^,R (Tempo vs. Nota) |
|-------------------------------------------------------------------------|
|Cuisines | $0,100$ | $0,047$ |
| All Recipes | $0,080$ | $0,008$ |

Os gráficos de dispersão (Figura 2) confirmam visualmente a ausência de um padrão discernível, com a linha de regressão linear praticamente horizontal. Dito isto, é possível afirmar que os resultados são robustos e demonstram que a complexidade ($\hat{I}$) e o esforço temporal ($\hat{T}$) são fatores negligenciáveis na avaliação final das receitas. Isso implica que receitas simples (poucos ingredientes, rápidas) têm o mesmo potencial de excelência avaliativa que pratos complexos ou de preparo prolongado. A qualidade percebida é, portanto, guiada por outros fatores, como o sabor final e o sucesso na execução. 
